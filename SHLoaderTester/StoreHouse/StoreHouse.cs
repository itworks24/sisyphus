...
namespace SHCMParser.StoreHouse
{
    class StoreHouseAPI
    {
        ...
        public static void LoadCmTreeElements(DBModel db)
        {
            var cmTree = new CmTreeProc();
            cmTree.Execute();
            db.CmTree.AddRange(cmTree.result.Select(item => new CmTreeDB(item))
                                                            .Where(item => !db.CmTree.Any(element => item.cm_tree_rid == element.cm_tree_rid)));
            db.SaveChanges();
        }
